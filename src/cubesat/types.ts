...
export interface IStateVector {
  norad_cat_id: string; // Unique ICAO 24-bit address of the transponder in hex string representation.
  sat_id: string; // Callsign of the vehicle (8 chars). Can be null if no callsign has been received.
  tle0: string; // Country name inferred from the ICAO 24-bit address.
  tle1: string; // Unix timestamp (seconds) for the last position update. Can be null if no position report was received by OpenSky within the past 15s.
  tle2: string; // Unix timestamp (seconds) for the last update in general. This field is updated for any new, valid message received from the transponder.
